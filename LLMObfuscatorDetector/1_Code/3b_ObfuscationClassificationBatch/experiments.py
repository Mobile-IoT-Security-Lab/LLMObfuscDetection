...
import AppUtils
import LLMUtils
from AnalysisUtils import AnalysisUtils, ObfuscationClassificationAnalysisUtils
...
def buildArgumentParser():
	parser = FriendlyArgumentParser(
		description="Run multiclass obfuscation classification campaigns using the OpenAI Batch API.",
		...
		epilog=(
			...
			"    --models gpt-5-mini \\\n"
			"    --prompt-ids ObfuscationClassificationV1 \\\n"
...
def queueRequestsForRunApp(runState, sampledApp):
	...
	for smaliIdx, smaliClass in enumerate(sampledApp["sampledClasses"]):
		prompt = ObfuscationClassificationAnalysisUtils.buildClassificationPrompt(promptTemplate, smaliClass, OBFUSCATION_TECHNIQUES)
		...
		for chunkIdx, (requestChunk, chunkSizeBytes) in enumerate(chunkRequests(pendingRequests, BATCH_MAX_REQUESTS, BATCH_MAX_FILE_BYTES, attemptIdx), start=1):
			...
			for request in requestChunk:
				...
				try:
					...
					label = ObfuscationClassificationAnalysisUtils.parseTechniqueLabel(rawReply, OBFUSCATION_TECHNIQUES)
...
def aggregateAppResult(appState):
	...
	for classInfo in appState["classes"].values():
		if len(classInfo["labels"]) != NUM_ITERATIONS:
		...
		classLabelCounts = {}
		for labelInfo in classInfo["labels"]:
			label = labelInfo["label"]
			classLabelCounts[label] = classLabelCounts.get(label, 0) + 1
		lastIterationLabel = classInfo["labels"][-1]["label"]
		majorityLabel = max(OBFUSCATION_TECHNIQUES, key=lambda label: (classLabelCounts.get(label, 0), label == lastIterationLabel))
		appLabelCounts[majorityLabel] = appLabelCounts.get(majorityLabel, 0) + 1
		lastClassLabel = majorityLabel

	effectiveNumSmaliClassesAnalyzed = appState["effectiveNumSmaliClassesAnalyzed"]
	llmFinalLabel = max(OBFUSCATION_TECHNIQUES, key=lambda label: (appLabelCounts.get(label, 0), label == lastClassLabel))
...
def printResultsSummary(results):
	stats = ObfuscationClassificationAnalysisUtils.computeResultsStatistics(results)
	predictedLabelCounts = {}
	for result in results:
		if result["status"] == "SUCCESS":
			label = result["llmFinalLabel"]
			predictedLabelCounts[label] = predictedLabelCounts.get(label, 0) + 1
	print("\n--- 📊 Dataset Statistics")
	...
	print("--- 🔹 Predicted Labels        : {}".format(predictedLabelCounts))
...
def main():
	...
	if len(allRequests) > 0:
		...
		batchName = "ObfuscationClassificationBatch_{}".format(datetime.datetime.now().strftime("%Y%m%d_%H%M%S"))

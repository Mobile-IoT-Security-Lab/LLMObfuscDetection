...
func main() {
	...
	func APKHunt_core_log(apkpath string) {
	    ...
	    log_file_path := apk_file_name + `.txt`
	    html_file_path := apk_file_name + `.html`
        ...
        // JSON output - salva solo basename.json nella directory corrente
        apkname_json := strings.TrimSuffix(filepath.Base(apkpath), filepath.Ext(filepath.Base(apkpath)))
        json_file_path := apkname_json + `.json`
        jsonData := map[string]interface{}{
                "apk":                   apkpathbase,
                "exported_activities":   cmd_and_exp_actv_output_count,
                "exported_providers":    cmd_and_exp_cont_output_count,
                "exported_receivers":    cmd_and_exp_brod_output_count,
                "exported_services":     cmd_and_exp_serv_output_count,
                "shared_preferences":    countSharedPref,
                "sqlite_db":             countSqliteDb,
                "firebase_db":           countFireDB,
                "realm_db":              countRealmDB,
                "internal_storage":      countIntStorage,
                "external_storage":      countExtStorage,
                "temp_files":            countTempFile,
                "logs":                  countLogs,
                "screenshots_protected": countScreenShots,
        }
        jsonBytes, json_err := json.MarshalIndent(jsonData, "", "  ")
        if json_err == nil {
                ioutil.WriteFile(json_file_path, jsonBytes, 0644)
                log.Printf("\n[+] JSON report saved: %s", json_file_path)
        }
